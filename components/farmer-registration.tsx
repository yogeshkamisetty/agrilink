...
export function FarmerRegistration({ onRegistered }: { onRegistered?: (farmer: Farmer) => void }) {
  ...
  return <section className='mx-auto max-w-5xl rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-8'>
    ...
    {step === 'form' && <form onSubmit={register} className='grid gap-5 md:grid-cols-2'>
      ...
      <button disabled={busy} className='rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition hover:opacity-90 disabled:opacity-60 md:col-span-2'>{busy ? 'Saving registration…' : 'Create profile and continue'}</button>
    </form>}
