...
export function IdentityVerification({ subjectType = 'farmer', subjectId = '00000000-0000-0000-0000-000000000001' }: Props) {
  ...
  return <section className='rounded-3xl border border-border bg-card p-4 shadow-sm sm:p-7'>
    <div className='flex flex-wrap items-start justify-between gap-4'><div><p className='text-xs font-semibold uppercase tracking-[.2em] text-primary'>Trust & safety</p><h2 className='mt-2 font-serif text-2xl sm:text-3xl'>Identity verification</h2><p className='mt-2 max-w-2xl text-xs sm:text-sm leading-6 text-muted-foreground'>A practical identity check for {subjectType}s. This validates format and consent only; it is not Aadhaar or government authentication.</p></div><span className='rounded-full bg-muted px-3 py-2 text-xs font-medium'>{phase === 'complete' ? 'Verified' : `Step ${phase === 'details' ? '1' : '2'} of 2`}</span></div>
    ...
    {phase === 'details' && <form onSubmit={start} className='mt-7 grid gap-4 sm:gap-5 md:grid-cols-2'>
      ...
      <label className='md:col-span-2 flex cursor-pointer gap-3 rounded-2xl border border-border p-3.5 sm:p-4 text-xs sm:text-sm leading-6'><input type='checkbox' checked={ownerConsent} onChange={e => setOwnerConsent(e.target.checked)} className='mt-1 size-4 accent-primary shrink-0' />I confirm that this Aadhaar number belongs to me.</label>
      <label className='md:col-span-2 flex cursor-pointer gap-3 rounded-2xl border border-border p-3.5 sm:p-4 text-xs sm:text-sm leading-6'><input type='checkbox' checked={storageConsent} onChange={e => setStorageConsent(e.target.checked)} className='mt-1 size-4 accent-primary shrink-0' />I consent to AgriLink storing my identity-verification information securely.</label>
      <label className='md:col-span-2 rounded-2xl border border-dashed border-border p-3.5 sm:p-4 text-sm'><span className='flex items-center gap-2 font-medium'><FileUp className='size-4 text-primary' />Optional Aadhaar card front</span><input type='file' accept='image/jpeg,image/png,application/pdf' onChange={e => setDocument(e.target.files?.[0] ?? null)} className='mt-3 block w-full text-xs text-muted-foreground file:mr-3 file:rounded-lg file:border-0 file:bg-primary/10 file:px-3 file:py-2 file:text-primary' /><span className='mt-2 block text-xs text-muted-foreground'>JPG, PNG or PDF · up to 5 MB · stored in a private Supabase bucket.</span></label>
      {error && <p className='md:col-span-2 rounded-xl bg-destructive/10 px-4 py-3 text-sm text-destructive'>{error}</p>}
      <button disabled={busy || !valid} className='md:col-span-2 min-h-[44px] rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-60'>{busy && <Loader2 className='mr-2 inline size-4 animate-spin' />}{busy ? 'Preparing verification…' : 'Send OTP'}</button>
    </form>}
    {phase === 'otp' && <><form onSubmit={verify} className='mx-auto mt-7 max-w-md space-y-4'><div className='rounded-2xl bg-muted p-4 text-sm'><p className='font-semibold'>Code sent to {mobile}</p><p className='mt-1 text-muted-foreground'>The code expires after 5 minutes.</p>{devOtp && <p className='mt-2 text-xs text-primary'>Local development code: {devOtp}</p>}</div><label className='text-sm font-medium'>One-time code<input required inputMode='numeric' value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} className={`${inputClass} text-center text-2xl tracking-[.4em]`} placeholder='123456' /></label>{error && <p className='rounded-xl bg-destructive/10 px-4 py-3 text-sm text-destructive'>{error}</p>}<button disabled={busy || otp.length !== 6} className='w-full min-h-[44px] rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground disabled:opacity-60'>{busy ? 'Verifying…' : 'Verify mobile'}</button><button type='button' disabled={busy} onClick={resend} className='w-full min-h-[40px] text-sm font-semibold text-primary disabled:opacity-60'>Resend OTP</button></form>{verification && <VerificationCard verification={verification} />}</>}
