...
export function AICopilotPanel() {
  ...
  return <section className="mt-6 grid gap-6 xl:grid-cols-[1.25fr_1fr]">
    <div className="rounded-xl border border-primary/30 bg-primary p-4 sm:p-5 text-primary-foreground">
      ...
      <button onClick={askCopilot} disabled={busy || !question.trim()} className="mt-3 inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground disabled:opacity-60">{busy ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />} Ask Copilot</button>
