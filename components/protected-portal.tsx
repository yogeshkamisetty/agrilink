...
export function ProtectedPortal() {
  ...
  useEffect(() => { let live = true; getAuthClient().auth.getSession().then(async ({ data }) => { if (!live) return; if (!data.session) return router.replace('/login'); const { data: profile } = await getAuthClient().from('user_profiles').select('onboarding_complete').eq('id', data.session.user.id).maybeSingle(); if (!profile?.onboarding_complete) return router.replace('/onboarding'); setReady(true) }).catch(() => router.replace('/login')); return () => { live = false } }, [router])
  ...
  return <><button onClick={signOut} className='fixed bottom-5 right-5 z-50 inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 text-sm font-semibold shadow-sm hover:bg-muted'><LogOut className='size-4' />Sign out</button><AgriLinkDashboard /></>
}
